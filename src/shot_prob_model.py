...
def xgb_classifier(max_depth=3, max_leaves=8, learning_rate=0.1, **kwargs):
...
if __name__ == '__main__':
    ...
    if find_best_features:
        best_features, best_score = get_best_feature_subset(
            basic_xgb_model, df, features, target, verbose=True
        )
        print('Best features:', best_features)
        print('Model score using best features:', best_score)
    else:
        # Best features is everything except "period"
        print('Using best features found based on previous runs:', features[1:])
